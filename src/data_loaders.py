...
def load_geobench(dataset, batch_size, channels=None, root_dir = './data', valid_split = 0):
    root_dir = root_dir + '/geobench'
    valid_split = 0.2 if valid_split else 0
    if dataset == 'bigearthnet':
        return load_bigearthnet(channels, batch_size, root_dir, valid_split)
    elif dataset == 'brickkiln':
        return load_brick_kiln(channels, batch_size, root_dir, valid_split)
    elif dataset == 'eurosat':
        return load_EuroSAT(channels, batch_size, root_dir, valid_split)
    elif dataset == 'so2sat':
        return load_so2sat(channels, batch_size, root_dir, valid_split)
    elif dataset == 'forestnet':
        return load_forestnet(channels, batch_size, root_dir, valid_split)
    elif dataset == 'pv4ger':
        return load_pv4ger(channels, batch_size, root_dir, valid_split)
    else:
...
def load_nucleotide_transformer(batch_size, one_hot = True, valid_split=-1, dataset_name = 'enhancers', split_state=42):
    ...
    if valid_split > 0:
      train_sequences, validation_sequences, train_labels, validation_labels = train_test_split(train_sequences, train_labels, test_size=0.1, random_state=split_state)
...
def load_cifar(root_dir, num_classes, batch_size, permute=False, seed=1111, valid_split=-1):
    if num_classes == 10:
    ...
    train_dataset = cifar(root=root_dir, train=True, transform=transforms.Compose(train_transforms), download=True)
    test_dataset = cifar(root=root_dir, train=False, transform=transforms.Compose(val_transforms))

    if valid_split > 0:
        valid_dataset = cifar(root=root_dir, train=True, transform=transforms.Compose(val_transforms))
...
def load_mnist(root_dir, batch_size, permute=False, seed=1111, valid_split=-1):
    ...
    train_dataset = datasets.MNIST(root=root_dir, train=True, download=True, transform=transforms.Compose(train_transforms))
    test_dataset = datasets.MNIST(root=root_dir, train=False, download=True, transform=transforms.Compose(val_transforms))

    if valid_split > 0:
        valid_dataset = datasets.MNIST(root=root_dir, train=True, transform=transforms.Compose(val_transforms))
        train_sampler, valid_sampler = split_dataset(train_dataset, len(test_dataset) / len(train_dataset))
...
def load_spherical(root_dir, batch_size, valid_split=-1):

    with gzip.open(os.path.join(root_dir, 's2_cifar100.gz'), 'rb') as f:
...
def load_deepsea(root_dir, batch_size, one_hot = True, valid_split=-1):
    filename = os.path.join(root_dir, 'deepsea_filtered.npz')
...
def load_darcy_flow(root_dir, batch_size, sub = 5, arch = None, valid_split=-1):
    ntrain = 1000
    ntest = 100
    r = sub # 5, 3, 2, 1
    h = int(((421 - 1)/r) + 1)
    s = h
    TRAIN_PATH = os.path.join(root_dir, 'piececonst_r421_N1024_smooth1.mat')
    TEST_PATH = os.path.join(root_dir, 'piececonst_r421_N1024_smooth2.mat')
...
def load_psicov(root_dir, batch_size, valid_split=-1):
    training_window = 128
    expected_n_channels = 57
    pad_size = 10

    all_feat_paths = [os.path.join(root_dir, 'protein/deepcov/features/'), os.path.join(root_dir, 'protein/psicov/features/'), os.path.join(root_dir, 'protein/cameo/features/')]
    all_dist_paths = [os.path.join(root_dir, 'protein/deepcov/distance/'), os.path.join(root_dir, 'protein/psicov/distance/'), os.path.join(root_dir, 'protein/cameo/distance/')]

    deepcov_list = load_list(os.path.join(root_dir, 'protein/deepcov.lst'), -1)

    length_dict = {}
    for pdb in deepcov_list:
        (ly, seqy, cb_map) = np.load(os.path.join(root_dir, 'protein/deepcov/distance/' + pdb + '-cb.npy'), allow_pickle = True)
        length_dict[pdb] = ly

    psicov_list = load_list(os.path.join(root_dir, 'protein/psicov.lst'))
    psicov_length_dict = {}
    for pdb in psicov_list:
        (ly, seqy, cb_map) = np.load(os.path.join(root_dir, 'protein/psicov/distance/' + pdb + '-cb.npy'), allow_pickle = True)
...
def load_music(root_dir, batch_size, dataset, length=65, valid_split=-1):
    if dataset == "JSB":
        # print('loading JSB data...')
        data = scipy.io.loadmat(os.path.join(root_dir, 'music/JSB_Chorales.mat'))
    elif dataset == "Muse":
        # print('loading Muse data...')
        data = scipy.io.loadmat(os.path.join(root_dir, 'music/MuseData.mat'))
    elif dataset == "Nott":
        # print('loading Nott data...')
        data = scipy.io.loadmat(os.path.join(root_dir, 'music/Nottingham.mat'))
    elif dataset == "Piano":
        # print('loading Piano data...')
        data = scipy.io.loadmat(os.path.join(root_dir, 'music/Piano_midi.mat'))
...
def load_ecg(root_dir, batch_size, valid_split=-1):
    window_size = 1000
    stride = 500

    # read pkl
    with open(os.path.join(root_dir, 'challenge2017.pkl'), 'rb') as fin:
        res = pickle.load(fin)
    ## scale data
    all_data = res['data']
...
def load_satellite(root_dir, batch_size, valid_split=-1):
    path = root_dir
...
def load_ninapro(root_dir, batch_size, arch, valid_split=-1):

    path = os.path.join(root_dir, 'ninaPro')
...
def load_cosmic(root_dir, batch_size, valid_split=-1):
    aug_sky=[-0.9, 3]
    path = os.path.join(root_dir, 'cosmic')
    ...
    if valid_split > 0:
        ...
        data_val = PairedDatasetImagePath(root_dir, val_dirs[::], aug_sky[0], aug_sky[1], part=None)
        val_loader = torch.utils.data.DataLoader(data_val, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    data_train = PairedDatasetImagePath(root_dir, train_dirs[::], aug_sky[0], aug_sky[1], part=None)
    data_test = PairedDatasetImagePath(root_dir, test_dirs[::], aug_sky[0], aug_sky[1], part=None)
...
def load_fsd(root_dir, batch_size, valid_split=-1):
...
class PairedDatasetImagePath(torch.utils.data.Dataset):
    def __init__(self, root_dir, paths, skyaug_min=0, skyaug_max=0, part=None, f_val=0.1, seed=1):
    ...
    def get_skyaug(self, i):
        ...
        sky_path = os.path.join(self.root_dir, path[2:], 'sky.npy') #JK
    ...
    def __getitem__(self, i):
        data = np.load(os.path.join(self.root_dir, self.paths[i][2:]))
        image = data[0]
        mask = data[1]
        if data.shape[0] == 3:
            ignore = data[2]
