...
class Chomp1d(nn.Module):
    ...
    def forward(self, x):
        if self.remove or self.chomp_size == 0:
            return x
...
class ResNet1D(nn.Module):
    ...
    def __init__(self, in_channels: int, mid_channels: int = 64,
                 ...
                 remain_shape=False, ks=None, ds=None, reshape_output=False, in_length=None, out_length=None) -> None:
        super().__init__()
        ...
        if self.reshape_output: # output is of different shape from input
            self.output = nn.Linear(in_length, out_length)
