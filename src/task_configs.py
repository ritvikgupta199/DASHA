...
from data_loaders import load_cifar, load_mnist, load_deepsea, load_darcy_flow, load_psicov, load_music, load_ecg, load_satellite, load_ninapro, load_cosmic, load_spherical, load_fsd, load_geobench, load_nucleotide_transformer
from data.bigearth_loader import load_BigEarthNet
from data.canadian_cropland_loader import load_cropland
from data.fmow_loader import load_fmow
...
def get_data(root_dir, dataset, batch_size, arch, valid_split, split_state = 42):
    ...
    if dataset == "your_new_task": # modify this to experiment with a new task
        ...
        train_loader, val_loader, test_loader = load_nucleotide_transformer(batch_size, one_hot = True, valid_split=valid_split, dataset_name = dataset, split_state=split_state)
...
def get_model(arch, sample_shape, num_classes, config_kwargs, ks = None, ds = None, dropout = None):
    ...
    if len(sample_shape) == 4:
        ...
        if 'mid_channels' in config_kwargs.keys() and config_kwargs['mid_channels'] is not None:
        ...
        if arch == 'your_new_arch': # modify this to experiment with a new architecture
            ...
            if 'reshape_output' in config_kwargs and config_kwargs['reshape_output']:
                in_length = sample_shape[2]
                out_length = config_kwargs['out_length']
                assert out_length is not None, "out_length must be specified for reshaping output"
                model = ResNet1D(in_channels = in_channel, mid_channels=mid_channels, num_pred_classes=num_classes, dropout_rate=dropout, ks = ks, ds = ds, activation=activation, remain_shape=remain_shape, reshape_output=True, in_length=in_length, out_length=out_length)
            else:
...
def get_hp_configs(dataset, n_train, arch):
    config = {
        "random_seed": tune.randint(0, 500),
        "lr": tune.loguniform(1e-5, 5e-1),
        "drop_rate": tune.choice([0, 0.05, 0.1]),
        "weight_decay": tune.loguniform(5e-7, 5e-3),
        "momentum": tune.uniform(0.9, 1)
    }
    return config, 20, 1.0
